...
def show_pendulum_move(StateHistory):
    ...
    Ax.set_xlim(-2, 2*PendelumsWidth)
    Ax.set_ylim(-2, 2*PendelumsHeight)
    ...
    def loop_animation(i):
        for j in range(PendelumsCount):
            ...
            x_offset = 2*(j%PendelumsHeight)
            y_offset = 2*(j//PendelumsHeight)

            Lines[j].set_data([
                y_offset, y_offset + L1*np.sin(Q1History[j,i]), y_offset + L1*np.sin(Q1History[j,i]) + L2*np.sin(Q2History[j,i])
            ], [
                x_offset, x_offset - L1*np.cos(Q1History[j,i]), x_offset - L1*np.cos(Q1History[j,i]) - L2*np.cos(Q2History[j,i])
            ])
    ...
    # ani.save("multy2_double_pendulum.mkv", writer='ffmpeg')
    plt.show()
